...
namespace Java.Interop
{
	...
	partial class NativeMethods {
		const string JavaInteropLibrary = "Urho3D";
	...
	public partial class JniRuntime : IDisposable
	{
		...
		protected JniRuntime (CreationOptions options)
		{
			...
			//TBD elix22 
			options.ValueManager = new AndroidValueManager();
			//TBD elix22 
			options.MarshalMemberBuilder = new MarshalMemberBuilder();
			SetValueManager (options);
			SetMarshalMemberBuilder (options);
			// TBD elix22
			ObjectReferenceManager      = SetRuntime (new AndroidObjectReferenceManager());
