...
{
	public class UIActionManager : IDisposable
	{
		...
		static UIElement[] tmpKeysArray = new UIElement[128];

		readonly Dictionary<UIElement, HashElement> targets = new Dictionary<UIElement, HashElement>();
		...
		public BaseAction GetAction(int tag, UIElement target)
		{
		...
		public ActionState GetActionState(int tag, UIElement target)
		{
			System.Diagnostics.Debug.Assert(tag != (int)ActionTag.Invalid);
		...
		public int NumberOfRunningActionsInTarget(UIElement target)
		{
			HashElement element;
			if (targets.TryGetValue(target, out element))
			{
		...
		public void Update(float dt)
		{
			...
			{
				tmpKeysArray = new UIElement[tmpKeysArray.Length * 2];
			}
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (!currentTarget.Paused)
				{
						...
						currentTarget.ActionIndex++)
					{
						...
						{
							// The currentAction told the UIElement to remove it. To prevent the action from
		...
		public void PauseTarget(UIElement target)
		{
			HashElement element;
			if (targets.TryGetValue(target, out element))
			{
		...
		public void ResumeTarget(UIElement target)
		{
			HashElement element;
			if (targets.TryGetValue(target, out element))
			{
		...
		public void ResumeTargets(List<UIElement> targetsToResume)
		{
		...
		public ActionState AddAction(BaseAction action, UIElement target, bool paused = false)
		{
		...
		public void RemoveAllActionsFromTarget(UIElement target)
		{
			if (target == null)
			{
				return;
		...
		internal void RemoveAction(BaseAction action, UIElement target)
		{
		...
		public void RemoveAction(int tag, UIElement target)
		{
			System.Diagnostics.Debug.Assert((tag != (int)ActionTag.Invalid));
			System.Diagnostics.Debug.Assert(target != null);
