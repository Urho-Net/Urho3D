...
- (void)setSDLWindow:(SDL_Window *)window 
{
    ...
    if (window) {
        ...
        if (@available(iOS 16, *)) {
            ...
            NSUInteger  orients = [data.uiwindow.rootViewController supportedInterfaceOrientations];
            
            UIInterfaceOrientation currentOrientation = [UIApplication sharedApplication].statusBarOrientation;
            
            BOOL supportsLandscape = (orients & (UIInterfaceOrientationMaskLandscape)) != 0;
            BOOL supportsPortrait = (orients & (UIInterfaceOrientationPortrait|UIInterfaceOrientationMaskPortraitUpsideDown)) != 0;

            /* Make sure the width/height are oriented correctly */
            if (!supportsPortrait &&(currentOrientation == UIDeviceOrientationPortrait || currentOrientation == UIDeviceOrientationPortraitUpsideDown)) {
                UIWindowScene * windowScene = data.uiwindow.windowScene;
                UIInterfaceOrientationMask _orientationPreferences = UIInterfaceOrientationMaskLandscapeRight;
                UIWindowSceneGeometryPreferencesIOS* preference =
                                    [[UIWindowSceneGeometryPreferencesIOS alloc]
                                    initWithInterfaceOrientations:_orientationPreferences];

                [windowScene requestGeometryUpdateWithPreferences:preference errorHandler:^(NSError* error)
                    {
                        SDL_SetError("requestGeometryUpdateWithPreferences failed");
                    }];
                
                [data.uiwindow.rootViewController setNeedsUpdateOfSupportedInterfaceOrientations];
            }
            else if (!supportsLandscape &&(currentOrientation == UIDeviceOrientationLandscapeLeft || currentOrientation == UIDeviceOrientationLandscapeRight)) {
                UIWindowScene * windowScene = data.uiwindow.windowScene;
                UIInterfaceOrientationMask _orientationPreferences = UIInterfaceOrientationMaskPortrait;
                UIWindowSceneGeometryPreferencesIOS* preference =
                                    [[UIWindowSceneGeometryPreferencesIOS alloc]
                                    initWithInterfaceOrientations:_orientationPreferences];
