...
EMSCRIPTEN_KEEPALIVE int
mono_wasm_assembly_invoke_static_main_method(MonoAssembly *mainMonoAssembly)
{
	int result = -1;
	MonoImage * image = mono_assembly_get_image (mainMonoAssembly);
	if(image != NULL)
	{
        ...
        for (int i = 0; i < rows; i++)
        {
			...
			{
                MonoClass* mainMonoClass = mono_wasm_assembly_find_class(mainMonoAssembly, name_space, name);
                if (mainMonoClass != NULL)
                {
                    printf("%s  %d  Class Program found  \n ", __FUNCTION__, __LINE__);

                    MonoMethod* mainMonoMethod = mono_wasm_assembly_find_method(mainMonoClass, "Main", 1);

                    if (mainMonoMethod != NULL)
                    {
                        printf("%s  %d  Method Main found  \n ", __FUNCTION__, __LINE__);
                        void* args[1];
                        mono_wasm_invoke_method(mainMonoMethod, NULL, args, NULL);
						result = 0;
						break;
                    }
                    else
                    {
                        printf("%s  %d  Method  not Main found  \n ", __FUNCTION__, __LINE__);
                    }
                }
                else
                {
                    printf("%s  %d  Class Program not  found  \n ", __FUNCTION__, __LINE__);
                }
            }
        }
    }
