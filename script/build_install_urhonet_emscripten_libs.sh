...
cd ${URHO3D_HOME}
./script/cmake_emscripten_dotnet_lib.sh build-emscripten-dotnet-libs
cd build-emscripten-dotnet-libs
make -j4
exit_status=$?
if [ $exit_status -ne 0 ]; then
    echo "An error occurred while executing build-emscripten-dotnet-libs.sh Exiting."
    exit 1 
fi
...
cp -f ${URHO3D_HOME}/build-emscripten-dotnet-libs/lib/libUrho3D.a ${URHONET_HOME_ROOT}/template/libs/web/Urho3D.a