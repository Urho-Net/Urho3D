...
# For some  reason On macOS,it is limited only to arm64 and x64 , they fixed it in the recent .NET10 preview 3
sed -i '' 's/if(TARGET_ARCH_NAME MATCHES "^(arm64|x64)$")/if(TARGET_ARCH_NAME MATCHES "^(arm64|x64|arm|x86)$")/' eng/native/tryrun.cmake
...
cp -rf artifacts/bin/native/net9.0-android-Release-arm/*.so  ${URHONET_HOME_ROOT}/template/libs/android/armeabi-v7a/

cp -rf artifacts/bin/mono/android.arm64.Release/*.so  ${URHONET_HOME_ROOT}/template/libs/android/arm64-v8a/
cp -rf artifacts/bin/native/net9.0-android-Release-arm64/*.so  ${URHONET_HOME_ROOT}/template/libs/android/arm64-v8a/

cp -rf artifacts/bin/mono/android.x64.Release/*.so  ${URHONET_HOME_ROOT}/template/libs/android/x86_64/
cp -rf artifacts/bin/native/net9.0-android-Release-x64/*.so  ${URHONET_HOME_ROOT}/template/libs/android/x86_64/

cp -rf artifacts/bin/mono/android.x86.Release/*.so  ${URHONET_HOME_ROOT}/template/libs/android/x86/
cp -rf artifacts/bin/native/net9.0-android-Release-x86/*.so  ${URHONET_HOME_ROOT}/template/libs/android/x86/

# Assuming all assemblies are the same for each ABI
rm -rf ${URHONET_HOME_ROOT}/template/libs/dotnet/bcl/android/common/*
cp -rf artifacts/bin/runtime/net9.0-android-Release-arm64/*.dll ${URHONET_HOME_ROOT}/template/libs/dotnet/bcl/android/common/
