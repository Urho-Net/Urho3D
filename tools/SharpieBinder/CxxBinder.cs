...
namespace SharpieBinder
{
    class CxxBinder : AstVisitor
    {
        ...
        {
            // Console.WriteLine("(" + caller +" : "+ lineNumber + ") " + message);
        }
        ...
        public CxxBinder(string outputDir)
        {
            baseNodeTypes = new Dictionary<string, BaseNodeType>
            {
                ...
                ["Urho3D::UIBatch"] = new BaseNodeType(Bind),
                ["Urho3D::Sphere"] = new BaseNodeType(Bind),
                ["Urho3D::Spline"] = new BaseNodeType(Bind),
                ["Urho3D::SceneResolver"] = new BaseNodeType(Bind),
                ["Urho3D::AreaAllocator"] = new BaseNodeType(Bind),
            };
            cbindingStream = File.CreateText(Path.Combine(outputDir, "binding.cpp"));
            pn("// Autogenerated, do not edit");
            pn("#include \"AllUrho.h\"");
            pn("#include \"interop.h\"");
        ...
        {
            ShowMessage($"ELI {typeDeclaration}");
        ...
        public override void VisitEnumDecl(EnumDecl decl, VisitKind visitKind)
        {
            ...
            {
                // TBD ELI , clear currentType in case of leaving the node.
        ...
        {
            ShowMessage($"ELI {decl} {visitKind} {currentType}");
            // We only care about enter visits, not leave
            if (visitKind != VisitKind.Enter)
            {
                ShowMessage($"ELI exit MethodIsBindable");
                return false;
            }
            ...
            {//variant return type is not support yet
                ShowMessage($"ELI exit MethodIsBindable");
        ...
        public override void VisitCXXMethodDecl(CXXMethodDecl decl, VisitKind visitKind)
        {
...
#if WEB_SUPPORT
			string pinvoke_name_csharp = pinvoke_name;
#endif
            var isConstructor = decl is CXXConstructorDecl;
            ...
            else
            {
                ...
                if(IsWebSpecialBinding(creturnType))
				{
                    ...
                    // C#
                    // pinvoke.ReturnType = AstType.Create(pinvokeReturn.ToString() + " *");
                    // pinvoke_name = "*" + pinvoke_name;
                    string newReturnType = "\n#if __WEB__\n" + pinvokeReturn.ToString() + " *" + "\n#else\n" + pinvokeReturn.ToString() + "\n#endif\n";
                    string new_pinvoke_name = "\n#if __WEB__\n" + "*" + pinvoke_name + "\n#else\n" + pinvoke_name + "\n#endif\n";
                    pinvoke.ReturnType = AstType.Create(newReturnType);
                    pinvoke_name_csharp = new_pinvoke_name;
            ...
            //
#if WEB_SUPPORT
			var invoke = new InvocationExpression(new IdentifierExpression(pinvoke_name_csharp));
#else
